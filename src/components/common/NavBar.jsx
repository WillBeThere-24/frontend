  ...
  return (
    <>
      <nav className={`flex h-[10vh] sticky top-0 justify-between w-screen gpx-[100px] items-center px-8 py-12 md:px-24 md:py-10 font-normal text-sm font-montserrat`}>
        ...
        {isOpen && (<div className={`${`flex flex-col top-0 bg-wybt-white p-20 w-3/4 absolute right-0 h-screen sm:hidden gap-5   `} transform ${isOpen ? '' : 'translate-y-[-100vh'} transition-transform duration-500 ease-in-out`}  ><div className={`flex sm:hidden flex-col  z-10 gap-3`}>
          <NavLink to="">Event Details</NavLink>
          <NavLink to="">Dashboard</NavLink>
          <NavLink to="">Contact Us</NavLink>
        </div>
          <div >
            <NavLink ><button className={`bg-wybt-primary text-wybt-white /h-[58.9px] rounded-md py-2 px-10 gap-4 text-center flex sm:hidden `}>Register</button></NavLink>
          </div ></div>)}
      </nav ></>
  )
}
