...
const DashNavBar = () => {
  ...
  return (
    <nav className='flex justify-between h-[10vh] py-10 md:px-24 px-8 w-full items-center mb-14 sticky top-0 shadow-md bg-wybt-white'>
      ...
      <div
        className={`border-wybt-primary border justify-between w-auto ml-auto border-solid  h-10 items-center rounded-md gap-12 py-6 px-10 sm:flex hidden ${
          dashNavOpen ? "fldex sm:hiddden" : ""
        }`}
      >
...
<<<<<<< HEAD
      ...
      {dashNavOpen && (
        <div className={`shadow-md border justify-between w-auto border-solid  h-[50vh] items-center rounded-md gap-12 py-14 px-9 flex sm:hidden flex-col absolute right-0 top-4  bg-wybt-white ${dashNavOpen ? "fldex sm:hiddden" : ""}`}>
...
=======
      <div className='sm:hidden block z-50 '>
        <button onClick={handleDashNavOpen}>
          {dashNavOpen ? (
            <CloseMenuBtn className='size-8' />
          ) : (
            <Harmburger className='size-8' />
          )}
        </button>
      </div>
      {dashNavOpen && (
        <div
          className={`border-wybt-primary border justify-between w-auto ml-bauto border-solid  h-[50vh] items-center rounded-md gap-12 py-12 px-8 flex sm:hidden flex-col absolute right-3 top-4  bg-wybt-white ${
            dashNavOpen ? "fldex sm:hiddden" : ""
          }`}
        >
          <div className='gap-5 flex whitespace-nowrap text-nowrap flex-col'>
